...
class Window(QWidget):
    ...
    def init_ui(self):
        ...
        changeSaveDirectoryAction.triggered.connect(self.change_directory)
    ...
    def open_video(self):
        ...
        if filename != "":
            ...
            # clear panels
            # This section is superseded because clearPanels() totally removes the panels, and the new updates to add-panel-from-file should update the rating and timestamps from an existing file
            self.clearPanels()
        ...
        if self.save_directory:
            ...
            if os.path.exists(timestamp_file):
                with open(timestamp_file) as f:
                    lines = f.readlines()
                    #This will create a list of dicts with the following save format
                    saved_lines = [] # ?
                    # The save format of a timestamp file is
                    # "Label={} | Start={} | End={}"
                    # This unpacks each line into a dict
                    for line in lines:
                        line_dict = dict()
                        for entry in line.split("|"):
                            key, value = entry.split("=")
                            line_dict[key] = value
                        saved_lines.append(line_dict)
                ...
                for task_index in range(len(self.groupButtonlist[self.panel_index])):
                    ...
                    if self.tasklist[self.panel_index][task_index].text() in [line_dict["Label"] for line_dict in saved_lines]:
    ...
    # This function should NOT save any files
    def add_rating_panel_from_file(self):
        ...
        # If an existing rating file exists, load it and update the ratings on the panel
        if self.save_directory:
            rating_file = "{}/{}_ratings.txt".format(self.save_directory,self.form_title[panel_index].text())
    ...
    # This saves a single line at a time, but is run through Python functools partial, and opens files in addition mode
